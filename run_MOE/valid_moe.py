...
class XAS3DMOE(nn.Module):
    def __init__(self,out_channels=120,num_experts=6,k=3,min_layers=2,noisy_gating=True,  coef=1e-3,dropout=0.5,gate_dropout=0.2,heads=4,
        cutoff=8.0,
        num_layers=1,hidden_channels=256,num_radial=5,num_spherical=2,middle_channels=82,num_output_layers=5, 
        ):
        #256 3 0.0005 5 2 82 5
    ...
    #                   ,num_output_layers=num_output_layers,num_radial=num_radial,num_spherical=num_spherical)
        self.gate_model=XAS3Dabs(out_channels=num_experts,cutoff=cutoff,num_layers=num_layers,hidden_channels=hidden_channels,middle_channels=middle_channels,num_output_layers=num_output_layers,num_radial=num_radial,num_spherical=num_spherical)#gate_model
        self.experts = torch.nn.ModuleList()#experts        
        for i in range(min_layers,num_experts+min_layers):
            self.experts.append(   XAS3Dabs(out_channels=out_channels,cutoff=cutoff,num_layers=num_layers,hidden_channels=hidden_channels,middle_channels=middle_channels,num_output_layers=num_output_layers,num_radial=num_radial,num_spherical=num_spherical)   )         
    ...
    def gating(self, data, noise_epsilon=1e-2):
        """Noisy top-k gating.
        """
        clean_logits = self.gate_model(data)
        logits=clean_logits
    ...
    def forward(self,data):
        gates = self.gating(data)


        expert_outputs = []
        for i in range(self.num_experts):
          input_data = data
          output=self.experts[i](input_data)
          expert_outputs.append(output)
        tmp=expert_outputs.copy()
...
def f(params,irun,str_save):
    ...
    model = XAS3DMOE(out_channels=120,num_experts=6,k=3 ).to(device)
